...
export function ProfileChangeConfirmationDialog({
  title,
  description,
  confirmLabel,
  busyLabel,
  ...
  return (
    <div className={styles.dialogBackdrop} onMouseDown={handleBackdrop}>
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="profile-change-confirm-title"
        aria-describedby="profile-change-confirm-description"
        aria-busy={isBusy}
        onKeyDown={keepFocusInDialog}
      >
        <header>
          <span>İşlem onayı</span>
          <h2 id="profile-change-confirm-title">{title}</h2>
        </header>
        <div className={styles.confirmBody}>
          <div
            className={danger ? styles.dangerNotice : styles.confirmNotice}
            id="profile-change-confirm-description"
          >
