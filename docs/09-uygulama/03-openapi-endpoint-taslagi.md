...
| POST | `/api/v1/platform/tenants` | P3K doğrulandı | Ayrı `PlatformBearerAuth` + canlı platform session; provisioning `{data,meta}`; status/id caller tarafından verilemez; optional configured limit |
| GET | `/api/v1/platform/tenants` | P3K doğrulandı | Ayrı `PlatformBearerAuth`; deterministic opaque cursor + bounded `limit`; yalnız projected platform metadata/health/configured limit |
| GET | `/api/v1/platform/tenants/{tenant_id}` | P3K doğrulandı | Ayrı `PlatformBearerAuth`; `{data,meta}` içinde yalnız tenant metadata/plan/region/lifecycle health/configured limit; HR veri/count yok |
| PATCH | `/api/v1/platform/tenants/{tenant_id}` | P3K doğrulandı | Ayrı `PlatformBearerAuth`; typed metadata/plan/status/limit update ve terminal lifecycle guard |
| GET | `/api/v1/platform/tenants/{tenant_id}/features` | P3K doğrulandı | Ayrı `PlatformBearerAuth`; fixed effective flag catalog; HR veri/usage yok |
| PATCH | `/api/v1/platform/tenants/{tenant_id}/features` | P3K doğrulandı | Ayrı `PlatformBearerAuth`; unique typed key + strict boolean rollout update |
...
- Historical F1E exact snapshot'ında altı platform route'u `x-required-principal: platform`, dört
  current-tenant route'u `x-required-principal: tenant` taşırdı. P3D aktif altı platform-tenant
  operasyonunu, platform audit ve `/platform/me` ile birlikte ayrı `PlatformBearerAuth` audience'ına
  taşıdı; dört tenant metadata/settings/feature compatibility route'u trusted-principal seam'ini
  korur. F2/P3 tenant-session yüzeyleri ve P3K ile migrate edilen 12 historical operation standard
  `BearerAuth` taşır. Public credential exchange endpointleri security scheme ilan etmez.
- Historical Faz 1 checkpoint'inde platform route'ları injected immutable `PlatformPrincipal`,
  tenant route'ları injected immutable `TenantPrincipal` istiyordu. Aktif platform dependency'si
  yalnız canlı platform-realm session'ını aynı dar principal seam'ine adapte eder; tenant
  compatibility dependency'si default deny kalır ve testler trusted seam'i override eder.
  Caller-supplied
...
- OpenAPI dokümanı okunabilir tag kataloğu kullanır: `System`, `Public`, `Authentication`,
  ...
  değerini korur. P3K metadata ve smoke gate'leri gerçek tenant-session operation'larında
  `BearerAuth`, platform-session yüzeyinde `PlatformBearerAuth` bulunduğunu ve public credential
  exchange route'larına yayılmadığını doğrular.
...
- W4B3 kapsamında bu taslak, mevcut FastAPI response shape'ine göre employee ve leave API'leri
  için concrete request/response örneklerini güncel tutar. Örnekler method/path, bearer session,
...
- P3K dashboard/employee/leave endpointleri tenant'ı yalnız doğrulanmış access token ve canlı
  session'dan alır. Caller `X-Tenant-Id`/`X-Tenant-Slug` değerleri OpenAPI parametresi değildir ve
  scope'u değiştirmez; header-only istek `401` olur.
...
- F2 activation, tenant-aware login, server-side refresh rotation/reuse detection, logout,
  ...
  tenant metadata/settings/feature compatibility route'ları fail-closed injected principal
  seam'ini korur; platform route'ları P3D'nin ayrı canlı platform session'ını kullanır. P3K'den
  sonra aktif HR HTTP yüzeyinde tenant header authorization mekanizması kalmaz.
- Authentication/authorization dependency hataları, API edge'de merkezi olarak map edilen typed
...
- W4A6 itibarıyla employee, leave balance ve leave request endpointlerinde public hata mesajları
  kod içi ortak sabitlerden üretilir; P3K regresyonları authentication'ın payload/query validation
  öncesinde fail closed olduğunu, null employee status lifecycle mesajını,
...
- Şu an kullanılan error code değerleri: `platform_access_denied`, `tenant_access_denied`,
  ...
  `tenant_closed`, `authentication_required`, `authorization_denied`,
...
- Employee listesinde `limit` + `Employee.id ASC` cursor uygulanmıştır. Devam varsa
...
Eksik, malformed, platform-audience veya revoke tenant credential `401` döner. Örnek:
...
- F2/P3 tenant operation'ları gerçek kısa ömürlü access credential için OpenAPI `BearerAuth`
  taşır. Login/activation public credential exchange, refresh/logout HttpOnly cookie yaşam
  döngüsüdür; platform login/me/audit ayrı `PlatformBearerAuth` realm'indedir.
...
Yalnız ayrı `PlatformBearerAuth` ile doğrulanmış canlı platform session'ından türetilen principal
typed rollout override yazabilir:
...
Opaque cursor payload'ı yalnız `id: UUID` taşır. Sıra `Employee.id ASC` ile deterministiktir ve
devam predicate'i tam olarak `Employee.id > cursor.id`'dir. Ek alanları yasaklayan schema eski
`employee_number` ve `created_at` cursor'larını reddeder; güncellenebilir `employee_number`
cursor'a girmez. Global `sort` ayrı backlog'dur.
...
Eksik/geçersiz tenant session varsa employee endpointleri payload/path validation hatalarından
önce `authentication_required` `401` zarfını döner.
...
| GET | `/api/v1/employees/{id}/leave-balances` | `leave:read:tenant` | Yetkili çalışanın manuel bakiye özeti |
...
Eksik/geçersiz tenant session varsa leave request endpointleri payload/query validation
hatalarından önce `authentication_required` `401` zarfını döner.
